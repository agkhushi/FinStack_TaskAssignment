...
import { Task, TaskFormInput, TaskFormInputSchema } from "@/lib/schemas";
import { suggestTaskTagsAction } from "@/lib/actions";
import { AlertCircle, Loader2, Sparkles, XIcon } from "lucide-react";
...
export function TaskFormDialog({
  ...
  const handleSuggestTags = async () => {
    ...
    try {
      ...
      // Automatically add suggested tags to current tags
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 p-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Task Time (HH:MM)</FormLabel>
                    <FormControl>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 min-h-[2.5rem] p-2 border rounded-md bg-background">
                {currentTags.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground">No tags added yet. Use "Suggest Tags" or add manually (future feature).</p>
                )}
              </div>
            </div>
